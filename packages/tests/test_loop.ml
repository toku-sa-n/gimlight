let () =
  let rendered = ref [] in
  let events = ref [] in
  let inputs = ref [ Game_input.Move Game_input.Left; Game_input.Quit ] in
  ...
  | [ initial; moved ] ->
      ...
      if (initial.Game_view.player_x, initial.Game_view.player_y) <> (10, 5)
      then failwith "loop should render the initial state";
